/* $Id: IEMAllInstThree0f38-x86.cpp.h 112403 2026-01-11 19:29:08Z knut.osmundsen@oracle.com $ */
/** @file
...
FNIEMOP_DEF(iemOp_ptest_Vx_Wx)
{
    ...
    else
    {
        ...
        IEMOP_HLP_DONE_DECODING_NO_LOCK_PREFIX_EX(fSse41);
        IEM_MC_MAYBE_RAISE_SSE_RELATED_XCPT();
        IEM_MC_FETCH_MEM_SEG_U128_ALIGN_SSE(uSrc2, ICORE(pVCpu).iEffSeg, GCPtrEffSrc);
...
#define IEMOP_BODY_PMOV_S_Z(a_Instr, a_SrcWidth, a_fRegNativeArchs, a_fMemNativeArchs) \
    ...
    { \
        ...
        IEM_MC_MAYBE_RAISE_SSE_RELATED_XCPT(); \
        IEM_MC_PREPARE_SSE_USAGE(); \
        IEM_MC_FETCH_MEM_SEG_U## a_SrcWidth (uSrc, ICORE(pVCpu).iEffSeg, GCPtrEffSrc); \
    ...
    if (!IEM_IS_MODRM_REG_MODE(bRm))
    {
        ...
        switch (ICORE(pVCpu).enmEffOpSize)
        {
            case IEMMODE_16BIT:
                ...
                IEM_MC_CALC_RM_EFF_ADDR(GCPtrEffSrc, bRm, 0);
                IEMOP_HLP_DONE_DECODING_NO_LOCK_PREFIX();
                IEM_MC_FETCH_MEM_SEG_U16(uSrc, ICORE(pVCpu).iEffSeg, GCPtrEffSrc);
