/* $Id: HM.cpp 80457 2019-08-28 07:31:35Z ramshankar.venkataraman@oracle.com $ */
/** @file
...
static DECLCALLBACK(int) hmR3Load(PVM pVM, PSSMHANDLE pSSM, uint32_t uVersion, uint32_t uPass)
{
    ...
    for (VMCPUID idCpu = 0; idCpu < pVM->cCpus; idCpu++)
    {
        ...
        else
        {
            ...
            rc  = SSMR3GetU32(pSSM, &pVCpu->hm.s.Event.fPending);
            rc |= SSMR3GetU32(pSSM, &pVCpu->hm.s.Event.u32ErrCode);
            rc |= SSMR3GetU64(pSSM, &pVCpu->hm.s.Event.u64IntInfo);
