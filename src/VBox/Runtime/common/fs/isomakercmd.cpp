/* $Id: isomakercmd.cpp 68840 2017-09-23 00:50:40Z knut.osmundsen@oracle.com $ */
/** @file
...
static void rtFsIsoMakerCmdUsage(PRTFSISOMAKERCMDOPTS pOpts, const char *pszProgName)
{
    rtFsIsoMakerPrintf(pOpts,
                       ...
                       "\n"
                       "    All non-options that does not start with '@' are taken to indicate a file,\n"
                       "    directory, or similar that is should be added to the ISO image.  Directories\n"
                       "    are added recursively and content is subject to filtering options.\n"
                       "\n"
                       "    Since there can be up to six different namespaces on an ISO, it is handy\n"
                       "    to be able to control the names used in each and be able to exclude an\n"
                       "    object from one or more namespaces.  The --name-setup option specifies the\n"
                       "    file specification format to use forthwith.\n"
                       "\n"
                       "    The default setup is:\n"
                       "\n"
                       "        --name-setup iso+joliet+udf+hfs\n"
                       "\n"
                       "    Which means you specify one on-ISO name for all namespaces followed by '='\n"
                       "    and the source file system name.  Only specifying the source file system\n"
                       "    will add the file/dir/whatever to the root of the ISO image.\n"
                       "\n"
                       ...
                       "\n"
                       "    In the first case the file '/home/user/Documents/product-x-readme.txt' is\n"
                       "    added to the ISO image as '/docs/readme.txt' in all enabled namespaces.\n"
                       "    In the primary ISO 9660 namespace, the filename will by default be converted\n"
                       "    to upper case because it's required by the spec.\n"
                       "\n"
                       "    In the second case the file is added to the root under the name\n"
                       "    'product-x-readme.txt' in all namespaces.  Though, in the primary ISO 9660\n"
                       "    namespace the name will be transformed to apply with the current ISO level,\n"
                       "    probably uppercased, possibly truncated too.\n"
                       "\n"
                       "    Given  --name-setup iso,joliet,udf  you can specify the name individually\n"
                       "    for each of the three namespace, if you like.  If you omit any, they will\n"
                       "    use last name given.  Any names left blank (==) will be considered omitted.\n"
                       "\n"
                       ...
                       "            /tmp/iso/real.txt\n"
                       "\n"
                       "    Using for instance  --name-setup udf  you can add a files/dirs/whatever to\n"
                       "    select namespace(s) without the more complicated empty name syntax above.\n"
                       "\n"
                       "    When adding directories, you can only control the naming and omitting of the\n"
                       ...
                       "        interpreted.  The specification is a comma separated list.  Each element\n"
                       "        in the list is a sub-list separated by space, '+' or '|' giving the\n"
                       "        namespaces that elements controls.  Namespaces are divied into two major\n"
                       "        and minor ones, you cannot specifying a minor before the major it\n"
                       "        belongs to.\n"
                       ...
                       "        corresponding --pop options is encountered.  The variations are for\n"
                       ...
                       "        current output buffer size (--output-buffer-size) as maximum.\n"
                       "\n"
