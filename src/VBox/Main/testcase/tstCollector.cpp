/* $Id: tstCollector.cpp 107623 2025-01-09 08:56:01Z andreas.loeffler@oracle.com $ */
/** @file
...
int main(int argc, char *argv[])
{
    ...
    if (RT_FAILURE(rc))
    {
        RTPrintf("tstCollector: RTR3InitExe() -> %Rrc\n", rc);
    ...
    if (netTest)
        testNetwork(collector);
    if (fsTest)
        testFsUsage(collector);
    if (diskTest)
        testDisk(collector);
